...
DIR = "/home/ubuntu/10k"
RAW_10K_DIR = "/home/ubuntu/efs/raw"
            ...
            if link.has_attr('name'):
                if link['name'] == item8_link.replace("#",''):
                    ...
                    if item8_contents_index >= 0:
                        ...
                        with open(PARSED_DIR + '/' + f , 'w') as outfile:
...
def parse_non_html(infile):
    ...
    try:
        match_line = target_lines.next()
        section_index = match_line.start(0)
        with open(PARSED_DIR+'/'+infile, 'w') as outfile:
...
def parse():
    ...
    print "HTML: %d parsed" % len(parsed_html_list)
    ...
    print "Non-HTML: %d parsed" % len(parsed_non_html_list)
    
    return None

def send_to_efs():
    ## copy all parsed files to one folder
    os.system("cp %s/* efs/parsed" % PARSED_DIR)
    return None

if __name__ == '__main__':
    ec2_IP = sys.argv[1]
    x = [line.strip() for line in open(DIR+'/10k-%s.txt' % ec2_IP,'r')]
    separate_html(x)
